...
def register_attention_control(unet):
    """
    In essence, register_attention_control injects custom attention control behavior into
    the U-Net model through the CustomizedAttnProcessor class.
    For every attn_proc it creates a new instance of CustomizedAttnProcessor that implements some attention mechanism.
    Finally, we switch the attn processors of the unet with the customized one that we've just created.
    """
    attn_procs = {}
    for name in unet.attn_processors.keys():
        attn_procs[name] = CustomizedAttnProcessor()
